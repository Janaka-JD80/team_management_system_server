...
async def create_project(project_in: ProjectCreate, db: AsyncSession = Depends(get_db), user: JwtPayload = Depends(require_manager)):
    return await project_service.create_project(db, name=project_in.name, description=project_in.description)

@router.put("/{project_id}", response_model=ProjectResponse)
async def update_project(project_id: str, project_in: ProjectUpdate, db: AsyncSession = Depends(get_db), user: JwtPayload = Depends(require_manager)):
    update_data = project_in.model_dump(exclude_unset=True)
    return await project_service.update_project(db, project_id, update_data)

@router.delete("/{project_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_project(project_id: str, db: AsyncSession = Depends(get_db), user: JwtPayload = Depends(require_manager)):
